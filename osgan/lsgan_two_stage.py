...
def train_epoch_adv(generator, discriminator, train_loader, optimizers):
    ...
    discriminator.cuda().train()                    #调用生成器和判别器

    optimizer_g, optimizer_d = optimizers               #定义优化
    ...
    for x, _ in train_loader:
        ...
        y_real = torch.ones(batch_size).cuda()           #张量并去相反数
        ...
        loss_real = loss_fn(pred_real,y_real)         #torch.mean()求平均值
        loss_fake = loss_fn(pred_fake,y_fake)

        loss_d = loss_real + loss_fake

        optimizer_d.zero_grad()             #初始化梯度
        loss_d.backward()
        optimizer_d.step()
        ...
        loss_g = loss_fn(pred_fake,y_real)
...
def parse():
    ...
    args.cfg = 'cfgs/wgan/wgan_dcgan_two_stage_celeba.yml'
    return args
